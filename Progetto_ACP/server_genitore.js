...
Studente.find({}).then((result) => {
    ...
    result.forEach((element) => {
        if (element.matricola > max) {
        max = element.matricola;
    }
    });
    newmatricola = max;
}).catch((err) => {
...
app.post("/effettuaIscrizione",(req,res)=>{
    ...
    Studente.find({codicefiscale:req.body.codicefiscale}).then((studentegiaiscritto)=>{
        ...
        if(studentegiaiscritto!=0){
            ...
            var newStudente = new Studente({matricola: Number(++newmatricola),nome: req.body.nome,cognome: req.body.cognome, codicefiscale: req.body.codicefiscale, datanascita: req.body.datanascita, classe: req.body.classe});
...
//GET: Visualizza iscrizione
app.get("/getIscrizione",(req,res)=>{
